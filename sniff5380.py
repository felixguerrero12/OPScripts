...
def find_url_from_headers(headers):
    host = ''
    uri = ''
    method = headers[0][:headers[0].find('/')].strip()
    for line in headers:
        # find host & uri
        if 'Host: ' in line:
            host = line.split('Host: ')[1].strip()
        if f'{method} /' in line:
            uri = line.split(f'{method} ')[1].split(' HTTP/')[0].strip()
    ...
    if pkt.haslayer(DNS):
        if pkt.getlayer(DNS).qr == 0:
            ...
            try:
                ...
                domain_rdata = " "
            print(f"{domain_request} translates to:  {domain_rdata}")
    elif pkt.haslayer(TCP) and pkt.getlayer(TCP).dport == 80 and pkt.haslayer(Raw):
        payload = pkt[Raw].load.decode('utf-8', 'ignore')
        print(
            f"{ip_src}:{str(pkt[TCP].sport)} - > {ip_dst}:{str(pkt[TCP].dport)} Full URL: {find_url_from_headers(extract_headers_from_request_payload(payload))}"
        )

    elif pkt[IP].proto == 6:
        print(
            f"TCP Communications: {pkt[IP].src}:{str(pkt[TCP].sport)} -> {pkt[IP].dst}:{str(pkt.getlayer(TCP).dport)}"
        )

    elif pkt[IP].proto == 17:
        print(
            f"UDP Communications: {pkt[IP].src}:{str(pkt[UDP].sport)} -> {pkt[IP].dst}:{str(pkt.getlayer(UDP).dport)}"
        )
